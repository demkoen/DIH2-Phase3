...
*   When reporting a vulnerability on the software, please, put in contact with the repository maintainers in order to discuss it in a private way.

## How to contribute

If you'd like to contribute, start by searching through the [issues](https://github.com/demkoen/DIH2-Phase3/issues) and [pull requests](https://github.com/demkoen/DIH2-Phase3/pulls) to see whether someone else has raised a similar idea or question.
...
[Issues](https://github.com/demkoen/DIH2-Phase3/issues) and [Pull Requests](https://github.com/demkoen/DIH2-Phase3/pulls) sections. Anybody is welcome to join these conversations.
...
```bash
git clone https://github.com/your-github-username/DIH2-Phase3.git
```

3.  Add the main DIH2-Phase3 repository as a remote to your forked repository (use any name for your remote repository, it does not have to be DIH2-Phase3, although we will use it in the next steps):

```bash
git remote add DIH2-Phase3 https://github.com/demkoen/DIH2-Phase3.git
```

Before starting contributing, remember to synchronize the `master` branch in your forked repository with the `master` branch in the main DIH2-Phase3 repository, by following this steps
...
```bash
git fetch DIH2-Phase3
```

3.  Merge them:

```bash
git rebase DIH2-Phase3/master
```

Contributions following this guidelines will be added to the `master` branch, and released in the next version. The release process is explained in the _Releasing_ section below.
...
-   Make sure all the dependencies have fixed versions.
...
5.  Create a new task for preparing the next release, adding the sufix `-next` to the current version number (to signal this as the development version).
